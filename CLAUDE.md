...
- `src/lib/email-gateway.ts` — Email Gateway client. **Adds no blind copy of its own, and must not.** Postmark bills per recipient and counts blind copies, so a standing staff BCC multiplies every send by the size of the staff list — on the free plan's 100-a-month cap that is the difference between shipping and stopping (July 2026: 628 API calls, 2,797 billed emails). A caller's `bccEmails` is forwarded exactly as supplied; a caller that supplies none sends no `bcc` at all. Internal visibility does not depend on it: Postmark keeps the full message in its Activity archive for 45 days and postmark-service writes a permanent metadata row per send. This has been re-proposed four times (#82, #92, #95, #106) — the answer is the archive, not a BCC.
