  ...
  .object({
    eventType: z.string().openapi({
      description:
        ...
        "Staff notification events (signup_notification, signin_notification, user_active, brand_daily_budget_changed, payment_method_removed, staff_daily_digest) are delivered to the internal staff recipient list, never to the customer.",
    }),
...
registry.registerPath({
  ...
  description:
    ...
    "- **No dedup** (all other event types, including brand_daily_budget_changed, payment_method_removed and staff_daily_digest): sends every time with no dedup.\n\n" +
    "**Staff routing:** `signup_notification`, `signin_notification`, `user_active`, `brand_daily_budget_changed`, `payment_method_removed` and `staff_daily_digest` are delivered to the internal staff recipient list instead of the customer resolved from `x-user-id`. Their metadata is enriched with the acting user's email under `email` when not already supplied — a caller with no acting user sends no actor metadata at all.\n\n" +
...
registry.registerPath({
  ...
  description:
    ...
    "**Accepted event types:** staff-bound events only (`signup_notification`, `signin_notification`, `user_active`, `brand_daily_budget_changed`, `payment_method_removed`, `staff_daily_digest`). Any other event type is rejected with 400, so no request on this path can reach a customer.\n\n" +
