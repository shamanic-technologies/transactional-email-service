...
import { releaseOperationId } from "./release-operation.js";
 ...
 * release's own tag, postmark-service already ingests Postmark's bounce and
 * complaint webhooks against it, and email-gateway aggregates exactly that set
 * — so one small request keyed on the release returns the counts a release is
 * judged on. `/public/stats/by-operation` takes no identity headers, which
 * matters because the worker has no request to take them from.
 *
 * It is keyed on the TAG rather than on the release's run for a reason worth
 * keeping: the run recorded against each message downstream is a child run
 * minted per send, not the release's run, so the run-keyed read matched nothing
 * and answered a well-formed zero. That is what made this probe inert for the
 * whole of v0.22.x.
 */
 ...
 * the next tick.
 */
export async function fetchDeliveryOutcomes(releaseId: string): Promise<DeliveryOutcomes> {
  if (!EMAIL_GATEWAY_SERVICE_API_KEY) {
    throw new Error("EMAIL_GATEWAY_SERVICE_API_KEY is not configured");
  }

  const operationId = releaseOperationId(releaseId);
  ...
  if (payload.matched !== true) {
    throw new Error(
      `email-gateway reports no message under operation ${operationId} — the question found nothing, which is not a verdict`
    );
  }
